...
class ResponseMeasurements:
    ...
    def __getMeasurements(self):
        ...
        self.__getSettlingMinIndex()
        self.__getSettlingMinTimeIndex()
    ...
    def __getSettlingMinIndex(self):
        self.settlingMinIndex = np.argmin(self.signal)
    
    def __getSettlingMinTimeIndex(self):           # Settling min time index
        if self.SP is None:
            # don't have reference SP 
            ss_low = self.signal[self.settlingMinIndex]
            n = len(self.signal)
            ...
            for i in range(0, len(self.signal)-self.start_measurement_index):
                if np.min(abs(np.asarray(self.signal[i:len(self.signal)-\
                    self.end_measurement_index]) - \
                        signal_end)) <= self.percentage * signal_end:
                    self.settlingMinTimeIndex = i
                    self.settlingTime = (i-self.inflectionTimeIndex)*abs(self.t[0] -\
                         self.t[1]) 
                    break
                else:
                    self.settlingMinTimeIndex = len(self.signal) \
    ...
    def __getSettlingTimeIndex(self):                 #original settling max time index 
        if self.SP is None:
            # don't have reference SP 
    ...
    def __getFallTime(self):
        ...
        timeTen = self.t[self.idxTen]
        timeNinety = self.t[self.idxNinety]
        self.fallTime = (timeTen - timeNinety)
