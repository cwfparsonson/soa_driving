...
class PSO:
    ...
    def __init__(self, 
                 ...
                 awg=None, 
                 osc=None, 
                 awg_res=8,         #how many voltage values it can take - verticle sampling
                 min_val=-1.0, 
                 max_val=1.0, 
        ...
        if os.path.exists(self.path_to_pso_data + 'curr_pos.csv') == True:
            ...
            # init analysis vals
            self.ft_rt_st_os_analysis = [self.__analyseSignal(self.gbest, curr_iter=0)]     #fall time added

            if self.record_extra_info == False:
                ...
                self.__savePsoData(np.copy(self.x), 
                                   ...
                                   self.gbest_cost_history, 
                                   [0], 
                                   self.ft_rt_st_os_analysis)                   #fall time added
    ...
    def __analyseSignal(self, OP, curr_iter):
        ...
        ft = responseMeasurementsObject.fallTime        #fall time added
        us = responseMeasurementsObject.undershoot
        ...
        os2 = responseMeasurementsObject.overshoot2


        return [ft, st, us, st_index, os2]
    ...
    def __savePsoData(self, 
                      ...
                      gbest_cost_history, 
                      iter_gbest_reached, 
                      ft_rt_st_os_analysis):                    #fall time added
        """
        ...
        - ft_rt_st_os_analysis = history of fall time, rise time, settling time and overshoot
        ...
        ft_rt_st_os_analysis_df = pd.DataFrame(ft_rt_st_os_analysis)        #fall time added
        ...
        gbest_cost_history_df.to_csv(self.path_to_pso_data + "gbest_cost_history.csv", 
                                     index=None, 
                                     header=False)
        ft_rt_st_os_analysis_df.to_csv(self.path_to_pso_data + 'ft_rt_st_os_analysis.csv',  #fall time added
                                    index=None, 
                                    header=False)
    ...
    def __getTransferFunctionOutput(self, tf, U, T, X0, atol=1e-12):
        ...
        #time_code = end_t - start_t

        #print("time for distort_tf "+ str(time_code))
    ...
    def __suppressAreasOfSignal(self, part_pos):
        ...
        if self.areas_to_suppress == 'None':
            ...
            start_drive = self.init_OP[0] 
            end_drive = self.init_OP[-1] 
            #max_drive = np.amin(self.init_OP) 
            for i in range(0, len(self.init_OP)):
                if self.init_OP[i] > start_drive:
                    K_index_signal_on = int(i - 0.5*(len(self.init_OP) - len(self.K))) 
                    break
    ...
    def __evaluateParticlePositions(self, particles, curr_iter=None, plot=False):
        ...
        for j in range(0, self.n): 
            ...
            if self.record_extra_info == True:
                ...
                curr_outputs = np.resize(curr_outputs,(self.num_points,self.upsampled))
                #print(curr_outputs)
        ...
        if plot == True:
            ...
            tenX = analyse.ResponseMeasurements(best_PV,self.t2).tenX
            tenY = analyse.ResponseMeasurements(best_PV,self.t2).tenY
            ninetyX = analyse.ResponseMeasurements(best_PV,self.t2).ninetyX
            ninetyY = analyse.ResponseMeasurements(best_PV,self.t2).ninetyY
            

            undershoot_index = analyse.ResponseMeasurements(best_PV,self.t2).minIndex
            undershoot = analyse.ResponseMeasurements(best_PV,self.t2).undershoot

            overshoot2_index = 80+ analyse.ResponseMeasurements(best_PV,self.t2).overshoot2_index

            #ss_low = analyse.ResponseMeasurements(best_PV, self.t2).SSLowValue
            
            # plt.plot(self.t2[Inflection], 
            #         best_PV[Inflection],
            #         marker='x', 
            #         markersize=6, 
            #         color="green", 
            ...
            # plt.plot(self.t2[index_Ninety], 
            #         best_PV[index_Ninety],
            #         marker='x', 
            #         markersize=6, 
            #         color="black", 
            #         label='Ninety Index')

            # plt.plot(self.t2[index_Ten], 
            #         best_PV[index_Ten],
            #         marker='x', 
            #         markersize=6, 
            #         color="blue", 
            ...
            plt.plot(self.t2[overshoot2_index], 
                     best_PV[overshoot2_index], 
                     marker='x', 
                     markersize=6, 
                     color="black", 
                     label='overshoot 2 index')
            
            plt.plot(self.t2[undershoot_index], 
                     best_PV[undershoot_index], 
                     marker='x', 
                     markersize=6, 
                     color="green", 
                     label='undershoot index')
            ...
            # ax.yaxis.set_ticks(np.arange(0, 0.00002, 0.0000025))
            # ax.yaxis.set_ticklabels(np.arange(-1, 0.5, 0.15))
            ...
            particles = np.resize(particles,(self.num_points,self.upsampled))
            #print(particles)
        ...
        #print("time for evaluate_particle_position " + str(time_code))
    ...
    def __runPsoAlgorithm(self):
        ...
        gbest_cost = np.copy(self.gbest_cost)
        gbest_cost_history = np.copy(self.gbest_cost_history)
        ft_rt_st_os_analysis = np.copy(self.ft_rt_st_os_analysis)       #fall time added
        iter_gbest_reached = np.copy(self.iter_gbest_reached)
        ...
        while curr_rep <= self.rep_max:
            ...
            while curr_iter <= self.iter_max:
                ...
                if pbest_value[min_cost_index] < gbest_cost_history[-1]:
                    for g in range(0, self.m):
                        gbest[g] = pbest[min_cost_index, g]
                    ft_rt_st_os_analysis = np.vstack((ft_rt_st_os_analysis,         #fall time added
                ...
                #print('fall time {}'.format(self.fallTime))
            ...
            gbest_cost_history = np.append([gbest_cost_history], [gbest_cost])
            self.ft_rt_st_os_analysis = np.vstack((ft_rt_st_os_analysis,               #fall time added
                                                self.__analyseSignal(gbest, 
                                                                     curr_iter)))
            ...
            gbest_cost_history = []
            gbest_cost_history = np.append([gbest_cost_history], [gbest_cost])
            ft_rt_st_os_analysis = [self.__analyseSignal(gbest, curr_iter)]     #fall time added
            iter_gbest_reached = np.copy(self.iter_gbest_reached)
    ...
    def __getPsoPerformance(self, 
        ...
        st_index = int(ft_rt_st_os_analysis[len(ft_rt_st_os_analysis)-1, 3])        #fall time added
        #print(ft_rt_st_os_analysis)
        ...
        # plot how rt, st and os varied with iterations         #fall time added
        max_rt, max_st, max_os = np.amax(ft_rt_st_os_analysis[:,1]),\
                                 np.amax(ft_rt_st_os_analysis[:,2]),\
                                 np.amax(ft_rt_st_os_analysis[:,3]) 
        plt.figure()
        plt.plot(iter_gbest_reached,        
                 ft_rt_st_os_analysis[:,1]/max_rt,      #fall time added
                 label='Rise Time')
        plt.plot(iter_gbest_reached, 
                 ft_rt_st_os_analysis[:,2]/max_st,          #fall time added
                 label='Settling Time')
        plt.plot(iter_gbest_reached, 
                 ft_rt_st_os_analysis[:,3]/max_os,              #fall time added
                 label='Overshoot')
...
if __name__ == '__main__':
    ...
    #num_points_list = [40,48,60,80,120,240]
    num_points_list = [240]   
    time_start = 0.0
    time_stop = 20e-9 
    ...
    # config pso params
    #driving_signals_list = [160, 180, 200, 220, 240, 260, 280, 300]   #number of input driving signals 
    #driving_signals_list = [3, 4, 5, 6, 7, 8, 9, 10]   #number of input driving signals 
    n = 30
    iter_max = 30
    rep_max = 1  
    max_v_f = 0.05
    ...
    if sim == True:
        ...
        x = 1
        while(x<=10):
            #for n in driving_signals_list:
            for num_points in num_points_list:   
                counter = 0
                directory = ('/Users/hadi/Desktop/SOA_Complexity/PSO_Data/Fall/Test/awg_res/n={}'.format(n) + '/points={}'.format(num_points) +'/Run={}'.format(x))
                            # set dir to save data  
                            # /Users/hadi/Desktop/SOA_Complexity/PSO_Data/Fall/Test/undershoottest/n= - local directory
                            # /home/uceelkh/Desktop/SOA_Complexity/PSO_Data/points={} - mammoth directory  
                            # /home/uceelkh/Desktop/SOA_Complexity/PSO_Data/Fall/New_23_04/n={} - mammoth directory 2  
                
                if os.path.exists(directory):  #check if directory is exists and remove existing directory with new run
                    shutil.rmtree(directory)
                    #directory =  directory + '_run={}'.format(x) 
                ...
                init_OP[:int(0.25*num_points)],init_OP[int(0.25*num_points):] = 0.5, -1    #fall of step
                ...
                p2p = upsampled/num_points        # p2p is the ratio of input output (240) points to the actual points of input signal - for upsampling

                t = np.linspace(time_start,time_stop,upsampled)                       #hardcode time parameter to 240 points
