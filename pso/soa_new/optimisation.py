...
class PSO:
    ...
    def __runPsoAlgorithm(self):
        ...
        while curr_rep <= self.rep_max:
            ...
            gbest_cost_history = np.append([gbest_cost_history], [gbest_cost])
            self.rt_st_os_analysis = np.vstack((ft_rt_st_os_analysis,               #fall time added
                                                self.__analyseSignal(gbest, 
                                                                     curr_iter)))
...
"""
        ...
        max_rt, max_st, max_os = np.amax(ft_rt_st_os_analysis[:,0]),\
                                 np.amax(ft_rt_st_os_analysis[:,1]),\
                                 np.amax(ft_rt_st_os_analysis[:,2]) 
        plt.figure()
        plt.plot(iter_gbest_reached,        
                 ft_rt_st_os_analysis[:,0]/max_rt,      #fall time added
                 label='Rise Time')
        plt.plot(iter_gbest_reached, 
                 ft_rt_st_os_analysis[:,1]/max_st,          #fall time added
                 label='Settling Time')
        plt.plot(iter_gbest_reached, 
                 ft_rt_st_os_analysis[:,2]/max_os,              #fall time added
                 label='Overshoot')
